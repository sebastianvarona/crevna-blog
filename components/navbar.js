...
export default function Navbar(props) {
    ...
    return (
        <nav className="bg-background text-primary border-b border-border fixed top-0 left-0 w-screen shadow-md z-10">
            <div className="container max-w-7xl mx-auto px-2 md:px-6 lg:px-8 relative">
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="md:hidden h-7 relative w-12 translate-x-1/2 ">
                        <Link href={`/${props.lang}`}>
                            <Image src='/images/logo.svg' alt="crevna" layout="fill" objectFit="contain"/>
                        </Link>
                    </div>
                    {/* Desktop */}
                    <div className="flex-1 hidden md:flex min-h-full">
                        {/* Logo */}
                    ...
                    {/* Globe */}
                    <div className="flex h-full items-center md:border-l border-border relative">
                        <button onClick={() => setLangDisplay(!langDisplay)} className={langDisplay ? "flex items-center ml-6 p-2 text-accent" : "flex items-center ml-6 p-2 text-primary hover:text-accent active:text-accent"} aria-label="change language">
                            ...
                            <span className="mx-1">{`${props.lang === 'en' ? 'En' : 'Es'}`}</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-chevron-down place-self-end pb-1" viewBox="0 0 16 16">
                        ...
                        <div className={ langDisplay ? "flex flex-col justify-center absolute right-0 top-14 bg-border rounded-md overflow-hidden" : "hidden" }>
                            <Link href="/"><a className="font-light hover:bg-secondary focus:bg-secondary py-2 px-12">English</a></Link>
                ...
                <div className={menuDisplay ? "bg-background flex flex-col w-full" : "hidden"}>
                    {links.map((item, index) => {
                        ...
                        <Link key={index} href={item.path}>
                            <a
                            className={`px-5 py-8 border-b border-border rounded-md font-semibold ${
                                router.pathname === item.path
                                ? 'text-accent underline underline-offset-8'
                                : 'hover:underline hover:underline-offset-8'
                            }`}
